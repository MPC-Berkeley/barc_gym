...
class RadiusArclengthTrack():
    ...
    #     # Makes sure s is within [0, track_length]
    #     sym_s_bar = ca.fmod(ca.fmod(sym_s, track_length) + track_length, track_length)
    #     # Piecewise constant function mapping s to track curvature
    ...
    #     sym_s_bar = ca.fmod(ca.fmod(s, track_length) + track_length, track_length)
    ...
    def get_barc3d_surface(self):
        from barc3d.surfaces.frenet_surface import FrenetSurface, FrenetSurfaceConfig
        surf = FrenetSurface()
        a = [self.key_pts[0,2]]
        for i in range(1, self.cl_segs.shape[0] + 1):
            l = self.cl_segs[i-1, 0]
            r = self.cl_segs[i-1, 1]
            if r == 0:
                psi = a[-1]
            else:
                theta = l / r
                psi = a[-1] + theta
            a.append(psi)
        config = FrenetSurfaceConfig(s=self.key_pts[:,3], a=a, y_min=-self.half_width, y_max=self.half_width, use_pchip=False, closed=True)
